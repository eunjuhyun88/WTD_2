...
def test_operator_decision_and_pattern_control_state_persist(tmp_path) -> None:
    db_path = tmp_path / "research_runtime.sqlite"
    store = ResearchStateStore(db_path)
    run = store.create_run(
        pattern_slug="tradoor-oi-reversal-v1",
    ...
    decision = store.record_operator_decision(
        ...
        decision="approve",
        decided_by="cto",
        rationale="Candidate is safe to train.",
        decided_at="2026-04-16T12:05:00+00:00",
    )
    assert decision.decision == "approve"

    control = store.upsert_pattern_control_state(
        "tradoor-oi-reversal-v1",
        updated_at="2026-04-16T12:06:00+00:00",
        auto_train_allowed=True,
        approval_required=False,
    )
    assert control.auto_train_allowed is True
    assert control.approval_required is False

    reloaded = ResearchStateStore(db_path)
    fetched_decision = reloaded.get_operator_decision(run.research_run_id)
    fetched_control = reloaded.get_pattern_control_state("tradoor-oi-reversal-v1")
    assert fetched_decision is not None
    assert fetched_decision.decided_by == "cto"
    assert fetched_control.auto_train_allowed is True
    assert fetched_control.approval_required is False
