...
def test_facts_indicator_catalog_returns_exactly_100_metrics() -> None:
    assert len(CATALOG) == 100

    response = _client().get("/facts/indicator-catalog")
...
def test_facts_indicator_catalog_supports_filters() -> None:
    response = _client().get(
        "/facts/indicator-catalog",
...
def test_facts_indicator_catalog_supports_stage_filters_and_next_gates() -> None:
...
def test_facts_indicator_catalog_maps_operational_and_cataloged_metrics() -> None:
...
def test_facts_indicator_catalog_route_rejects_invalid_filters() -> None:
    status_response = _client().get("/facts/indicator-catalog", params={"status": "wrong"})
    assert status_response.status_code == 400
    assert status_response.json()["detail"]["code"] == "invalid_status"

    family_response = _client().get("/facts/indicator-catalog", params={"family": "wrong"})
