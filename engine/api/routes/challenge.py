...
async def create_challenge(req: ChallengeCreateRequest) -> ChallengeCreateResponse:
    ...
    pattern_input = PatternInput(snaps=snaps, user_id=req.user_id)
    ...
    # --- 2. Build pattern vector from snaps ------------------------------
...
async def scan_challenge(slug: str) -> ChallengeScanResponse:
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Challenge not found: {slug}")
    ...
    for symbol in universe:
        ...
        # Score the LAST bar of each symbol.
