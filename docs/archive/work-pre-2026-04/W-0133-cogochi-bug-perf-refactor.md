...
현재 `/cogochi` 에서 확인된 **클릭 불가** 버그 1건 + **성능 이슈** 4건을 수정하고,
...
3. 3개 fetch (`analyze`, `venue-divergence`, `liq-clusters`) 는 완전히 독립적 — `Promise.allSettled` 로 병렬화 가능.
4. scan engine 은 18개 upstream 을 `Promise.allSettled` + 5s timeout 으로 이미 올바르게 처리 중.
5. `$shellStore` 는 마크업에서 이미 올바르게 subscribed (auto `$` 표기법).
