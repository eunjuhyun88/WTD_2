...
- B 축: capture_records (chart drag) + ledger_verdicts (3-cat)
- B 축: workspace pins, setups, research_contexts
- B 축: indicator_preferences (W-0123)
- A×B 합류: _on_entry_signal → CaptureRecord ★ (방금 픽스)
- 6-cat verdict 타입 ★ (방금)
- PatternObject evolution_chain ★ (방금)

### 8.2 BROKEN ⚠️ (실제로는 작동 안 함)

- ledger 702 rows = 모두 synthetic test data (2025-01-01 hardcoded, 합성)
  → 진짜 프로덕션 capture는 0건이었다 (위 픽스 전까지)
- LightGBM Layer C = 모델 미훈련 → fallback (0.60·A + 0.40·B)
- Wiki layer = 디렉토리 없음
- ContextAssembler = 클래스 없음 → 각 LLM call이 ad-hoc prompt 조립

### 8.3 NOT BUILT ❌ (Phase 1 우선순위)

- B 축: AI Parser route (A-03)
- B 축: Chart drag → PatternDraft (A-04, R-03)
- B 축: 1-click Watch (D-03, R-04)
- B 축: Verdict UI 6-cat 버튼 (F-02, R-01 후속)
- A×B: F-60 multi-period gate API (H-07, R-05/R-06)
- B 축: Notification 시스템 (notification_queue + 72h scheduler)
- Read: Wiki API + UI
- Read: Dashboard F-60 progress bar
- 인프라: Migration system (engine/db/migrations/)
- 인프라: Ledger Supabase 이전 (현재 JSON files DEPRECATED)
...
✅ Data Engine 마스터 정본 (W-data-engine-master)
✅ Data Pipeline 정본 (이 문서)
```

### Phase 1 다음 작업 (baseline 만들기)

1. **R-01 후속 UI**: Verdict 6-cat 버튼 (F-02) — capture가 inbox에 떨어지기 시작했으니 6-cat 버튼이 즉시 필요
2. **R-04 1-click Watch**: capture → monitoring 연결 (D-03)
3. **R-05 F-60 multi-period gate**: 200+ verdict 누적 시 측정 시작 (H-07)
4. **Migration system 도입**: schema 변경 가능하게
5. **Ledger Supabase 이전**: JSON files DEPRECATED → Supabase ledger_* 테이블
6. **Notification 시스템**: 72h verdict 알림 (자동 inbox 유도)

### Phase 2

- Wiki layer 전체 (`engine/wiki/`)
- ContextAssembler (`engine/agents/context.py`)
- AI Parser (A-03) + Chart drag (A-04, R-03)
