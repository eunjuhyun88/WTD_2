...
# 6개 historical 항목 (한 줄씩 — 날짜 jsonl + agent별 jsonl 양쪽에)
ENTRIES=(
...
'{"ts":"2026-04-26T17:00:00Z","id":"A006","event":"worktree 50->5 + agent docs PR #320","tags":["session","end","A006"],"importance":"high","branch":"docs/*","end_sha":"51ea37cf","shipped":"PR #314 #318 #320","handoff":"Multi-Agent OS v2 design saved -> A007 implements"}'
)

DATE_TARGET="memory/sessions/2026-04-26-agent-backfill.jsonl"

for entry in "${ENTRIES[@]}"; do
  AGENT_ID=$(echo "$entry" | jq -r '.id')
  AGENT_FILE="memory/sessions/agents/${AGENT_ID}.jsonl"

  # Idempotent — 이미 그 agent ID로 기록이 있으면 skip
  if [ -f "$AGENT_FILE" ] && grep -q "\"id\":\"$AGENT_ID\"" "$AGENT_FILE" 2>/dev/null; then
    continue
  fi

  echo "$entry" >> "$DATE_TARGET"
  echo "$entry" >> "$AGENT_FILE"
done

echo "✓ Backfilled A001-A006:"
echo "  date jsonl:  $DATE_TARGET"
echo "  agent jsonls: memory/sessions/agents/A00{1..6}.jsonl"
