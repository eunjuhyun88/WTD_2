  ...
  <div class="gmx-panel">
    ...
    {#if step === 'input'}
      ...
      {#if collateralNum > 0}
        <div class="gmx-summary">
          ...
          {#if !hasEnoughUsdc}
            <div class="sum-warn">⚠ Insufficient USDC balance</div>
          {/if}
          {#if !hasEnoughEth}
            <div class="sum-warn">⚠ Insufficient ETH balance (needed for gas)</div>
          {/if}
        </div>
      {/if}
    ...
    {:else if step === 'error'}
      <div class="gmx-status error">
        <span class="status-icon">✕</span>
        <p>{errorMsg}</p>
        <button class="gmx-submit" on:click={() => { step = 'input'; }}>Try Again</button>
      </div>
    {/if}
  </div>
</div>
