...
<section class="structure-explain" aria-label="Structure signal decode">
    <header class="se-head">
      ...
      <span class="se-sub">Structure/position context tagged by engine (not a confirmed pattern)</span>
    </header>
    ...
    {#if model.wyckoff}
      <div class="se-block" data-accum={model.wyckoff.isAccumulation ? '1' : '0'}>
        ...
        {/if}
        <p class="se-hint">
          Approximates events like SC→Spring→SOS from swing highs/lows, volume z-scores, and candle shape. Lit chips above were detected in the recent window.
        </p>
      </div>
    {/if}
    ...
    {#if model.mtf.length > 0}
      <div class="se-block">
        ...
        <p class="se-hint">
          Each timeframe OHLC is resampled separately and scored with the same rules. The [1H] label indicates the 1H result.
        </p>
      </div>
    {/if}
    ...
    {#if model.oi}
      <div class="se-block">
        ...
        {/if}
        <p class="se-hint">
          OI notional ÷ 24h volume. A high ratio means positions are over-stacked relative to trading turnover (leverage concentration). Funding sign adds short/long skew risk to the score (e.g. +12).
        </p>
      </div>
    {/if}
  </section>
