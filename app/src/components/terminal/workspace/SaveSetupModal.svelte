<script lang="ts">
  ...
  /**
   * SaveSetupModal — appears when user clicks `Save Setup` from terminal.
   * This flow is capture-first: reviewed-range evidence is saved through the
   * canonical terminal capture route and any downstream projection is explicit.
   */
  ...
  const hasViewportPreview = $derived(Boolean(viewportPreview?.klines?.length));
  ...
  async function handleSave() {
    ...
    if (!viewportAtSave?.klines?.length) {
      saveError = 'reviewed_range_required';
      return;
    }
    saving    = true;
    saveError = null;
    ...
    try {
      ...
      if (!captureRecord) {
        throw new Error('capture_record_missing');
      }
...
<div class="modal-backdrop" onclick={onClose} onkeydown={handleKeydown} role="dialog" aria-modal="true" tabindex="-1">
  ...
  <div class="modal" onclick={(e) => e.stopPropagation()} onkeydown={() => {}}>
    ...
    <div class="modal-header">
      <div class="header-left">
        ...
        {#if viewportPreview}
          ...
          >
        {:else}
          <span class="viewport-warning">저장할 차트 구간을 아직 읽지 못했습니다.</span>
        {/if}
      </div>
    ...
    {#if saveError}
      <p class="save-error">! {saveError === 'reviewed_range_required' ? '저장하려는 차트 구간을 먼저 확인해 주세요.' : saveError}</p>
    {/if}

    <!-- Actions -->
    <div class="modal-actions">
      <button class="cancel-btn" onclick={onClose}>취소</button>
      <button class="save-btn" onclick={handleSave} disabled={saving || !hasViewportPreview}>
        {saving ? '저장 중…' : 'Save Setup →'}
      </button>
    </div>
