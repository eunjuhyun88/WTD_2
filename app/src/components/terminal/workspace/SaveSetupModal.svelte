<script lang="ts">
  ...
  /**
   * SaveSetupModal — capture-first terminal save surface.
   * Save Setup persists a durable pattern capture with an explicit chart viewport,
   * then hands that capture id downstream into Lab.
   */
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (!captureRecord) throw new Error('capture_save_failed');
...
<div class="modal-backdrop" onclick={onClose} onkeydown={handleKeydown} role="dialog" aria-modal="true" tabindex="-1">
  ...
  <div class="modal" onclick={(e) => e.stopPropagation()} onkeydown={() => {}}>
    ...
    <div class="modal-actions">
      ...
      <button class="save-btn" onclick={handleSave} disabled={saving || !viewportPreview}>
        {saving ? '저장 중…' : '캡처 저장 →'}
      </button>
    </div>
