  ...
  async function runAnalysis() {
    ...
    try {
      const res = await fetch('/api/analyze', {
        ...
        body: JSON.stringify({ analysis: analysisText, providers: ALL_PROVIDERS }),
      });
      const data = await res.json();
      if (!res.ok) { error = data.error ?? 'Analysis failed'; return; }
...
<svelte:head>
  ...
  <meta name="description" content="Compare trading analysis text across multiple LLMs. Extract building block signals and derive consensus." />
  ...
  <header class="surface-hero surface-fixed-hero">
    <div class="surface-copy">
      ...
      <p class="surface-subtitle">
        Paste analysis text and multiple AIs will simultaneously extract building block signals for comparison.
      </p>
    </div>
    ...
    {#if hasResults && consensus}
      <div class="surface-stats">
        ...
        <article class="surface-stat">
          <span class="surface-meta">Common blocks</span>
          <strong>{consensus.blocks.length}</strong>
        </article>
        <article class="surface-stat">
          <span class="surface-meta">Models</span>
          <strong>{successResults.length} / {results.length}</strong>
        </article>
        <article class="surface-stat">
          <span class="surface-meta">Fastest</span>
  ...
  <div class="surface-scroll-body analyze-body">
    ...
    <section class="surface-grid">
      <div class="surface-section-head">
        ...
        <div class="input-actions">
          <div class="provider-badges">
            {#each PROVIDERS as p}
              <span class="provider-badge" style="--p-color:{p.color}">
                <span class="p-dot"></span>{p.label}
              </span>
            {/each}
          </div>
      ...
      <div class="surface-card input-card">
        <textarea
          ...
          placeholder="Paste your trading analysis text here&#10;&#10;e.g. CVD analysis, funding rate, OI change, L/S ratio, conclusion..."
          rows="10"
        ></textarea>
        <div class="input-footer">
          <span class="char-count">{analysisText.length} chars</span>
          <button
            class="surface-button run-btn"
            onclick={runAnalysis}
            disabled={loading || !analysisText.trim()}
          >
            {loading ? 'Analyzing…' : `Run analysis (${selectedProviders.length} models)`}
          </button>
        </div>
      </div>
    </section>
    ...
    {#if loading}
      <section class="surface-card page-loading">
        ...
        <span class="loading-text">Calling LLMs in parallel…</span>
      </section>
    {/if}
        ...
        <section class="surface-grid">
          <div class="surface-section-head">
            <div>
              ...
              <h2>Common signals — agreed by {Math.ceil(successResults.length / 2)}+ models</h2>
            </div>
            ...
            <div class="model-col surface-card" class:model-error={!!r.error}>
              ...
              {:else}
                ...
                <div class="model-section">
                  <span class="model-label">Bias</span>
                ...
                {#if r.entry_condition}
                  <div class="model-section">
                    <span class="model-label">Entry condition</span>
                ...
                {#if r.risk_note}
                  <div class="model-section">
                    <span class="model-label">Risk</span>
          ...
          <div class="surface-card heatmap-card">
            <div class="heatmap-grid">
              {#each Object.entries(blockCounts).sort(([, a], [, b]) => b - a) as [block, count]}
                ...
                <div class="heat-item" title="{block}: {count}/{successResults.length} models">
...
<style>
  ...
  .char-count { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.2); font-family: var(--sc-font-mono, monospace); }
  ...
  .dot-label { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.4); font-family: var(--sc-font-mono, monospace); }
  ...
  .model-latency { margin-left: auto; font-size: var(--ui-text-xs); color: rgba(255,255,255,0.3); font-family: var(--sc-font-mono, monospace); }
  .model-slug { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.2); font-family: var(--sc-font-mono, monospace); padding-left: 15px; }
  ...
  .model-label { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.25); font-family: var(--sc-font-mono, monospace); letter-spacing: 0.08em; text-transform: uppercase; }
  ...
  .conf-value { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.35); font-family: var(--sc-font-mono, monospace); min-width: 30px; text-align: right; }
  ...
  .block-name { font-size: var(--ui-text-xs); font-family: var(--sc-font-mono, monospace); color: rgba(255,255,255,0.7); }
  .block-count { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.3); font-family: var(--sc-font-mono, monospace); }
  .no-blocks { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.2); font-family: var(--sc-font-mono, monospace); }
