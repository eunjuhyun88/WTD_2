<script lang="ts">
  ...
  let status = $state<VerdictStatus>(
    (data.tokenStatus as VerdictStatus | 'ready') === 'ready' ? 'ready' : (data.tokenStatus as VerdictStatus),
  );
  ...
  const cardRotation = $derived(
    isSwiping ? Math.max(-3, Math.min(3, (swipeDeltaX / SWIPE_THRESHOLD) * 3)) : 0,
  );
  ...
  function onPointerUp(e: PointerEvent) {
    ...
    const velocity = Math.abs(delta) / Math.max(Date.now() - swipeStartTime, 1);
...
<svelte:head>
  ...
  {#if captureInfo}
    <meta name="description" content="Submit your verdict for {captureInfo.symbol} pattern capture." />
  {/if}
</svelte:head>
  ...
  {:else if status === 'ready' && captureInfo}
    ...
    >
      ...
      <div class="verdict-header">
        ...
        <p class="verdict-swipe-hint">← 불일치 &nbsp;|&nbsp; 일치 →</p>
      </div>
      ...
      <div class="verdict-grid">
        <button class="verdict-choice verdict-valid" disabled={submitting} onclick={() => submitVerdict('valid')}>
          <span class="verdict-emoji">✅</span>
          <strong>Valid</strong>
          <span>Profitable entry</span>
        </button>
        <button class="verdict-choice verdict-invalid" disabled={submitting} onclick={() => submitVerdict('invalid')}>
          <span class="verdict-emoji">✗</span>
          <strong>Invalid</strong>
          <span>Wrong pattern / Loss</span>
        </button>
        <button class="verdict-choice verdict-near-miss" disabled={submitting} onclick={() => submitVerdict('near_miss')}>
          <span class="verdict-emoji">~</span>
          <strong>Near Miss</strong>
          <span>Pattern correct, entry missed</span>
        </button>
        <button class="verdict-choice verdict-too-early" disabled={submitting} onclick={() => submitVerdict('too_early')}>
          <span class="verdict-emoji">⏫</span>
          <strong>Too Early</strong>
          <span>Entered too early</span>
        </button>
        <button class="verdict-choice verdict-too-late" disabled={submitting} onclick={() => submitVerdict('too_late')}>
          <span class="verdict-emoji">⏰</span>
...
<style>
  ...
  .verdict-card--swipeable:active { cursor: grabbing; }
  ...
  .verdict-icon { font-size: 2.5rem; color: #86efac; }

  .verdict-header { display: flex; flex-direction: column; gap: 4px; }
  .verdict-kicker { font-size: var(--ui-text-xs); text-transform: uppercase; letter-spacing: 0.1em; color: rgba(255,255,255,0.4); }
  .verdict-symbol { font-size: 1.8rem; font-weight: 700; letter-spacing: 0.02em; margin: 0; }
  .verdict-meta  { color: rgba(255,255,255,0.5); font-size: var(--ui-text-sm); margin: 0; }
  .verdict-error { color: #f87171; font-size: var(--ui-text-sm); margin: 0; }
  .verdict-swipe-hint { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.3); margin: 4px 0 0 0; }
  ...
  .verdict-grid > :last-child:nth-child(odd) { grid-column: 1 / -1; }
  ...
  .verdict-choice strong { font-size: var(--ui-text-sm); color: rgba(255,255,255,0.9); }
  .verdict-choice span   { font-size: var(--ui-text-xs); color: rgba(255,255,255,0.45); }
