...
function parseSignals(thesis: string): SeedSignal[] {
  ...
  // Fallback: if user wrote short thesis, still rank by momentum/volume.
...
function scoreScanCoin(coin: OpportunityScore, requested: SeedSignal[]): MatchCandidate {
  ...
  for (const signal of requested) {
    switch (signal) {
      case 'volume_breakout': {
        const hasVolumeTag = coin.alerts.some((a) => /거래량|volume|스파이크|spike/i.test(a));
      ...
      case 'short_funding_pressure':
      case 'long_funding_pressure': {
        // Opportunity scan does not carry direct OI/funding metrics in v1.
        missingSignals.push(signal);
        break;
      }
    }
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const candidates = [...boardMatches, ...scanMatches]
