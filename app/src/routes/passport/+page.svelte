    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          error = 'Connect your wallet or log in to view your Passport.';
          return;
        }
...
<div class="surface-page passport">
  ...
  {:else if passport}
    ...
    <section class="surface-card">
      ...
      {:else}
        <p class="body-copy">No badges yet. They will accumulate here as your verification history grows.</p>
      {/if}
    </section>
  {/if}
</div>
