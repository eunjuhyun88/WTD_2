<script lang="ts">
  ...
  function buildPath(curve: number[], allMin: number, allMax: number): string {
  ...
  const baselineY = $derived(
    H - ((1.0 - globalMin) / (globalMax - globalMin || 1)) * (H - 20) - 10,
  );
</script>
...
<div class="benchmark-chart-wrap">
  ...
  >
    ...
    {#if btcCurve && btcCurve.length >= 2}
      <path
        d={buildPath(btcCurve, globalMin, globalMax)}
        fill="none"
    ...
    {#each series as s, i}
      {#if s.curve.length >= 2}
        <path
          d={buildPath(s.curve, globalMin, globalMax)}
          fill="none"
