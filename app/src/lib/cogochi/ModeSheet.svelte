<script lang="ts">
  ...
  const modes = [
    ...
    { id: 'train',     icon: '◈', label: 'TRAIN',    sub: 'Desktop recommended',  locked: true,   color: 'var(--amb)' },
    { id: 'flywheel',  icon: '◉', label: 'FLYWHEEL', sub: 'Desktop recommended',  locked: true,   color: '#7aa2e0' },
  ] as const;
    ...
    <div class="ms-list">
      {#each modes as m}
        ...
        >
          ...
          {:else if m.locked}
            <span class="ms-lock">Desktop only</span>
          {/if}
        </div>
      {/each}
    </div>
