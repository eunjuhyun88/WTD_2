...
- H7 Axis-B (ARCH_ZONE composition) landed as its own atomic commit: `required_blocks=[]` with `required_any_groups=[["sideways_compression", "bollinger_squeeze", "volume_dryup"]]` — any single compression signal (classical sideways per Wyckoff; volatility-band squeeze per Bollinger 2001 / Hudson & Urquhart 2021; volume drying-up per Edwards & Magee 1948) can now open ARCH_ZONE. FAKE_DUMP advance blocks (`recent_decline`, `funding_extreme`) are deliberately excluded so the two phases stay structurally distinct. `min_bars=4` and the `oi_spike_with_dump` disqualifier are unchanged — that timing and entry guard are kept as separate future axes per the methodology. `_compute_confidence` was updated to count satisfied `required_any_groups` as contributions, so phases composed from OR-groups no longer collapse to confidence 0.0 on a valid advance.
- Real run `ade68a09-6a22-4ccc-8449-5acad3a215c1` on `2026-04-18` shows Axis-B moved TRADOOR's replay forward by two phases: the earlier `FAKE_DUMP (stuck)` progression is now `ARCH_ZONE -> REAL_DUMP -> FAKE_DUMP -> ARCH_ZONE`. ARCH_ZONE is entered (Axis-B worked), REAL_DUMP is reached (Axis-A OI=8% worked as part of the chain), but `REAL_DUMP.max_bars=4` times out before `ACCUMULATION` can form (TRADOOR's first Wyckoff-SOS bar is at `2026-04-11 21:00 UTC`, five hours after REAL_DUMP entry at `16:00`). FDR stays 1.0 but the remaining bind is now explicit: either extend `REAL_DUMP.max_bars`, or widen `ACCUMULATION.transition_window_bars`, or both — a combined Axis-A (threshold) + Axis-B (timing-window) adjustment grounded in the measured 5-hour ACCUMULATION-formation window on TRADOOR vs the current 4-hour REAL_DUMP timeout.
- H7 Axis-A round-2 landed two aligned OI-threshold changes anchored in the same Park-Hahn-Lee (2023) / Koutmos (2019) / Bessembinder-Seguin (1993) literature as the earlier `oi_spike_with_dump` change: `REAL_DUMP.max_bars` widened `4 -> 12` to match the 4-12h accumulation-formation window after liquidation cascades, and `oi_hold_after_spike.spike_threshold` was calibrated `12% -> 8%` so whale-hold detection is achievable on real altcoin perp data (the 12% default had `0` `oi_hold_after_spike` hits on TRADOOR's 12h post-dump window despite the actual OI ramping from `+2%` to `+24%` on `oi_change_24h`).
- **First core-loop closure**: the real run `8f12a57e-385d-4b70-bf26-4e806b5c07ea` on `2026-04-18` finally produces a complete phase sequence on TRADOOR holdout: `ARCH_ZONE -> REAL_DUMP -> ACCUMULATION -> BREAKOUT -> ARCH_ZONE`, with `target_hit=True`, `entry_hit=True`, and `28 target_hits` total across all evaluated variants (previously `0` across every run in this work-item's history). Winner is `tradoor-oi-reversal-v1__real-patience-accum-early` — a non-canonical variant that the search actually promoted ahead of canonical on merit. Promotion gate still rejects (FDR=0.5 above the 0.4 ceiling is the last remaining gate; phase_fidelity=0.7 >= 0.5 floor, reference_recall=1.0, holdout_passed=True), but the core loop is mechanically closed end-to-end for the first time and the FDR metric is now meaningfully measurable (was saturated at 1.0).
- **Edge measurement on the single closed case** (CTO-diligence check against sample-of-one overfitting): replay trace of three independent variants (`canonical`, `funding-bias-window16`, `compression-emphasis`) on TRADOOR holdout all converge on the same ACCUMULATION entry at `2026-04-12 00:00 UTC` @ close `3.482`, BREAKOUT at `2026-04-12 10:00 UTC` @ close `4.733`. Post-entry price trajectory: `entry+6h = -5.37%` (shallow drawdown), `entry+10h = +35.93%` (at BREAKOUT hit), `entry+12h = +38.54%`, `entry+24h = +55.74%`, `entry+48h = +60.71%`, `48h max-high gain = +111.00%`, `48h max-low drawdown = -20.82%`. The three variants converging on the same timestamp from different search origins is evidence this is a real detection, not a curve-fit artefact. This does not establish general alpha (still one event, one holdout) but does establish that the single closed case carries meaningful economic magnitude rather than being a 0% target-hit.
- **Honest caveat** carried forward to next slices: H6 (multi-symbol benchmark) is required before claiming generalisation. Cache inventory showed only `PTBUSDT` and `TRADOORUSDT` have 1h klines data locally; 331 perp-only symbols are available but need klines fetched. Online data-fetch is a separate session's work.
- **Parameter sensitivity sweep** on the five tuned thresholds (5 params × 5-7 values each, off-repo scratch tool, no artifacts stored to prod store) at the current closed-loop config confirms the tuning is NOT a knife-edge overfit. OI thresholds (`oi_spike_with_dump.oi_spike_threshold` and `oi_hold_after_spike.spike_threshold`, both currently `0.08`) show a plateau from `0.05` to `0.09` with identical metrics (FDR=0.5, phase_fidelity=0.7, 12 target_hits, holdout_passed=True) and a cliff at `0.10` where FDR jumps to `1.0` and target_hits drop to `0`. `REAL_DUMP.max_bars` (current `12`) shows a monotone ramp (`4 -> 6 hits`, `6 -> 9`, `10 -> 12`) and a plateau at `10-24`. `breakout_from_pullback_range.min_drawdown` (current `0.05`) is flat across `0.02-0.15`. `breakout_above_high.lookback_days` (current `5`) is completely flat across `2-20` — this is a real finding: the param is effectively dead code now that BREAKOUT uses `breakout_from_pullback_range` instead, and `breakout_above_high` no longer participates in any phase's required blocks.
- **Sensitivity interpretation (CTO honesty)**: plateau existence refutes knife-edge overfitting (`\u00b125%` perturbation on OI thresholds leaves the run identical). But the cliff at `0.10` on both OI thresholds sits exactly above TRADOORUSDT's full-history `oi_change_1h` max of `9.57%`, so the plateau is calibrated to TRADOOR's OI dynamic range, not proven-general across symbols. Robust within the symbol, generalisation still blocked on H6 multi-symbol. Dead-code cleanup of `breakout_above_high` and the unused `min_drawdown` range is a low-priority follow-up.
