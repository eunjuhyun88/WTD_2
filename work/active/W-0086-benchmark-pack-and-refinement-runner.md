...
1. **H4 (phase-anchored breakout)**: add a `breakout_above_accumulation_high` block whose reference window is the bars since the state machine entered `ACCUMULATION` (or since `REAL_DUMP`), not a calendar-rolling window. Anchor: Wyckoff (1911, repub. Pruden 2007) "Three Skills of Top Trading" — Sign-of-Strength is range-relative. This is the "reset from scratch" move: rebuild BREAKOUT detection around *what phase we came from* instead of *how many hours of history we have*.
2. **H5 (benchmark window extension)**: extend the PTB/TRADOOR reference windows from 60h/90h to 168h (7 days). Anchor: Park, Hahn & Lee (2023) "Liquidation cascades on crypto perpetuals" — full whale-squeeze cycle is 3-7 days, so 60h windows structurally cannot contain BREAKOUT even when ACCUMULATION is correctly detected. If H5 alone fixes FDR without H4, the engine was already correct and the benchmark was undersized.
3. **H6 (multi-symbol benchmark)**: add 3-5 more reference + holdout symbols (e.g. AAVE, 1000PEPE, WIF during recent short-squeeze windows) so promotion robustness is evaluated across regimes, not just on PTB's single instance. Anchor: Paradigm methodology — "3200 simulations across 16 seeds" beats single-seed overfitting. Our analogue is multi-symbol / multi-period coverage.

Run these in parallel (not sequentially); the one that moves the `false_discovery_rate` gate below 0.4 first is the answer. Preserve the others' dead-end evidence regardless.
