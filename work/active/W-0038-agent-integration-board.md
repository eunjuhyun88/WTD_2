...
- `main` is at `a3c3f13` after PR `#37`.
- PR `#43` (`codex/w-0035-pattern-transition-persistence`) is the first engine dependency and is mergeable.
- `codex/w-0036-terminal-persistence-rollout` and `codex/w-0037-pattern-capture-record` currently point at the same W-0035 commit base and are not independent implementation PRs yet.
...
1. Promote and merge PR `#43` after confirming checks.
2. Rebase or recreate `W-0037` from updated `main` and implement capture records.
...
- Verification status: queue created; PR `#43` tests previously passed locally.
- Remaining blockers: PR checks and merge execution for `#43`.
