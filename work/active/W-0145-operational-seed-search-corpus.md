...
1. align app search contracts and plane clients with canonical engine catalog/seed/scan payloads.
...
- branch: `codex/w-0145-search-proxy-client`
- worktree: `/private/tmp/wtd-v2-w0145-corpus-plane`
- verification: `npm --prefix app run test -- src/lib/server/enginePlanes/planeClients.test.ts src/lib/server/enginePlaneProxy.test.ts` = `8 passed`; `npm --prefix app run contract:check:engine-types` = passed; `npm --prefix app run check` = `0 errors`, pre-existing `111 warnings`
