...
- app/engine stats·refinement endpoints와 `scanEngine` hot path에 대해 최소 1회 timing capture를 남긴다.
- `npm --prefix app run check`
- `UV_CACHE_DIR=/tmp/wtd-v2-uv-cache uv run --directory engine python -m pytest tests/test_refinement_routes.py tests/test_ledger_store.py tests/test_pattern_candidate_routes.py -q`
...
- add explicit timing instrumentation for stats/refinement routes and `_runServerScanInternal`
- capture one comparison sample for `/api/patterns/stats`, `/api/refinement/leaderboard`, and scan total duration
- split the highest measured bottleneck into a separate implementation slice once timing data confirms the first patch target
...
- active work item: `work/active/W-0125-engine-perf-hotpath-measurement.md`
- branch/worktree: dedicated hotpath worktree exists at `/private/tmp/wtd-v2-w0125-stats-hotpath` on `codex/w0125-stats-hotpath`; current workspace branch is `claude/w-0126-ledgerstore-supabase`
- verification: bulk stats fan-out removal, record scan reduction, and refinement snapshot reuse are reflected here; explicit timing capture is still pending
- remaining blockers: no measured p95 or route/function timing snapshot is recorded yet
- 가장 느린 endpoint 후보: `/api/patterns/stats`
- 가장 큰 구조 병목: ledger JSON 재스캔
- 다음 패치 순서: measurement → stats reuse 확인 → refinement reuse 확인 → scan fan-out guard
